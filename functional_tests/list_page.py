...
class ListPage(object):
    ...
    def share_list_with(self, email):
        # Submitting an email in the "share"-textfield will acknowledge that mail.
    ...
    def wait_for_row_in_list_table(self, item_text, item_number):
        # The expected row text with a correct row number will pop up
        expected_row_text = f'{item_number}: {item_text}'
        rows = self.get_table_rows()
        self.test.assertIn(expected_row_text, [row.text for row in rows])

    def add_list_item(self, item_text):
        # Submitting a text in the item form will create and present that item
